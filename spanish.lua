local AceLocale = LibStub:GetLibrary("AceLocale-3.0")
local L = AceLocale:NewLocale("ElvUI", "esES") or AceLocale:NewLocale("ElvUI", "esMX")
if not L then return end

--ActionBars
L[" |cff00ff00bound to |r"] = " |cff00ff00ligado(a) a |r"
L["Action Paging"] = "Paginación de Acción"
L["ActionBars"] = "Barras de Acción"
L["All keybindings cleared for |cff00ff00%s|r."] = "Todas los atajos borrados para |cff00ff00%s|r."
L['Alpha'] = "Transparencia";
L["Anchor Point"] = "Punto de Fijación" --also in unitframes
L["Backdrop"] = "Fondo"
L["Bar "] = "Barra "
L["Binding"] = "Controles"
L["Binds Discarded"] = "Teclas Descartadas"
L["Binds Saved"] = "Teclas Guardadas"
L["Button Size"] = "Tamaño del Botón" --Also used in Bags
L["Button Spacing"] = "Espaciado de Botones" --Also used in Bags
L["Buttons Per Row"] = "Botones por Fila"
L["Buttons"] = "Botones"
L['Change the alpha level of the frame.'] = "Cambia el nivel de transparencia del cuadro";
L["Color when the text is about to expire"] = "Color del texto cuando esté a punto de expirar."
L["Color when the text is in the days format."] = "Color del texto cuando tenga formato de días."
L["Color when the text is in the hours format."] = "Color del texto cuando tenga formato de horas."
L["Color when the text is in the minutes format."] = "Color del texto cuando tenga formato de minutos."
L["Color when the text is in the seconds format."] = "Color del texto cuando tenga formato de segundos."
L["Cooldown Text"] = "Texto de Reutilización"
L["Days"] = "Días"
L["Discard"] = "Descartar"
L["Display bind names on action buttons."] = "Muestra las teclas asignadas en los botones."
L["Display cooldown text on anything with the cooldown spiril."] = "Muestra el texto de reutilización sobre todo lo que tenga la espiral de reutilización."
L["Display macro names on action buttons."] = "Muestra el nombre de las macros en los botones."
L["Expiring"] = "Expiración"
L["Height Multiplier"] = "Multiplicador de Altura"
L["Hours"] = "Horas"
L['Key Down'] = "Tecla Pulsada";
L["Key"] = "Tecla"
L["Keybind Mode"] = "Modo de Teclado"
L["Keybind Text"] = "Texto de Tecla"
L["KEY_ALT"] = "A"
L["KEY_BUTTON3"] = "M3"
L["KEY_CTRL"] = "C"
L["KEY_DELETE"] = "Del"
L["KEY_HOME"] = "Hm"
L["KEY_INSERT"] = "Ins"
L["KEY_MOUSEBUTTON"] = "M"
L["KEY_MOUSEWHEELDOWN"] = "MwD"
L["KEY_MOUSEWHEELUP"] = "MwU"
L["KEY_NUMPAD"] = "N"
L["KEY_PAGEDOWN"] = "PD"
L["KEY_PAGEUP"] = "PU"
L["KEY_SHIFT"] = "S"
L["KEY_SPACE"] = "SpB"
L["Low Threshold"] = "Umbral Bajo"
L["Macro Text"] = "Texto de Macro"
L['Max Paging'] = "Paginación Máxima";
L['Micro Bar'] = "Micro Barra";
L["Minutes"] = "Minutes"
L["Mouse Over"] = "Pasar el ratón sobre" --Also used in Bags
L["Multiply the backdrops height or width by this value. This is usefull if you wish to have more than one bar behind a backdrop."] = "Multiplica el ancho o alto de los fondos por este valor. Es útil se deseas tener más de una barra con fondo."
L["No bindings set."] = "No hay teclas establecidas."
L["Pet Bar"] = "Barra de Mascota"
L["Restore Bar"] = "Restaurar Barra"
L["Restore the actionbars default settings"] = "Restaura las barras de acción a los ajustes predeterminados."
L["Seconds"] = "Segundos"
L["Set the font size of the action buttons."] = "Establece el tamaño de la fuente de las barras de acción."
L["Stance Bar"] = "Barra de Forma";
L["The amount of buttons to display per row."] = "Número de botones a mostrar por fila";
L["The amount of buttons to display."] = "Número de botones a mostrar";
L['The button you must hold down in order to drag an ability to another action button.'] = "Botón que debes mantener presionado para mover una habilidad a otro botón de acción.";
L["The first button anchors itself to this point on the bar."] = "El primer botón se fija a este punto de la barra."
L["The size of the action buttons."] = "El tamaño de los botones de acción."
L["Threshold before text turns red and is in decimal form. Set to -1 for it to never turn red"] = "Umbral para que el texto se ponga rojo. Está en forma decimal. Establécelo en -1 para que nunca se ponga rojo"
L["Toggles the display of the actionbars backdrop."] = "Muestra/Oculta el fondo de las barras de acción."
L['Trigger'] = "Disparador"
L["Visibility State"] = "Estado de Visibilidad"
L['When enabled the main actionbar will use the maximum amount of pages available. This means if you have another actionbar disabled, the actionbar page will become available on the main actionbar. Having this disabled will limit the main actionbar to two pages.'] = true;
L["Width Multiplier"] = "Multiplicador de Ancho"
L[ [=[This works like a macro, you can run different situations to get the actionbar to page differently.
 Example: '[combat] 2;']=] ] = [=[Esto funciona como una macro. Puedes ejecutar diferentes situaciones para paginar la barra de acción de forma diferente.
 Ejemplo: '[combat] 2;']=]
L[ [=[This works like a macro, you can run different situations to get the actionbar to show/hide differently.
 Example: '[combat] show;hide']=] ] = [=[Esto funciona como una macro. Puede ejecutar diferentes situaciones para mostrar u ocultar la barra de acción de forma diferente.
 Ejemplo: '[combat] show;hide']=]

--Bags
L["Ascending"] = "Ascendente";
L["Bag Columns"] = "Columnas de las Bolsas";
L["Bag-Bar"] = "Barra de las Bolsas";
L["Bags"] = "Bolsas"
L["BAGS_DESC"] = "Adjust bag settings for ElvUI."
L["Bank Columns"] = "Columnas del banco";
L["Bar Direction"] = "Dirección de la Barra";
L["Bottom to Top"] = "De Abajo hacia Arriba";
L["Click to search.."] = "Haz clic para buscar..."
L["copperabbrev"] = "|cffeda55fc|r" --Also used in gold datatext
L["Delete Grays"] = "Borrar objetos grises"
L["Deleted %d gray items. Total Worth: %s"] = "Se borraron %d objetos grises. Valor total: %s"
L["Descending"] = "Descendiente";
L["Direction the bag sorting will use to allocate the items."] = "Dirección de ordenado que se usará para distribuir los objetos.";
L["Enable/Disable the all-in-one bag."] = "Habilitar/Deshabilitar la bolsa todo en uno."
L["Enable/Disable the Bag-Bar."] = "Activa/Desactiva la barra de las bolsas.";
L["goldabbrev"] = "|cffffd700g|r" --Also used in gold datatext
L["Hold Shift:"] = "Mantener Shift:"
L["No gray items to delete."] = "No hay objetos grises para eliminar."
L["No gray items to sell."] = "No hay objetos grises para vender."
L["Number of columns (width) of bags. Set it to 0 to match the width of the chat panels."] = "Número de columnas (anchura) de las bolsas. Establecelo a 0 para coincidir con el ancho de los paneles de chat.";
L["Number of columns (width) of the bank. Set it to 0 to match the width of the chat panels."] = "Número de columnas (anchura) del banco. Establecelo a 0 para coincidir con el ancho de los paneles de chat.";
L["Purchase"] = "Comprar"
L["Set the size of your bag buttons."] = "Establece el tamaño de tus botones de la bolsa.";
L["silverabbrev"] = "|cffc7c7cfs|r" --Also used in gold datatext
L["Sort Bags"] = "Ordenar Bolsas"
L["Sort Direction"] = "Dirección de Ordenado"; --Also used in Buffs and Debuffs
L["Sort Orientation"] = "Orientación de Ordenado";
L["Sort Special"] = "Ordenado Especial"
L["Stack Items"] = "Apilar Objetos"
L["Stack Special"] = "Apilado Especial"
L["The direction that the bag frames be (Horizontal or Vertical)."] = "La dirección que los cuadros de bolsas tienen (Horizontal o Vertical).";
L["The direction that the bag frames will grow from the anchor."] = "La dirección que los cuadros de bolsas crecerán desde el anclaje.";
L["The frame is not shown unless you mouse over the frame."] = "El cuadro no se muestra a menos que pases el ratón sobre él."
L["The spacing between buttons."] = "Espaciado entre los botones."
L["Toggle Bags"] = "Mostrar/Ocultar Bolsas"
L["Top to Bottom"] = "De Arriba hacia Abajo";
L["Vendor Grays"] = "Vender Objetos Grises"
L["Vendored gray items for:"] = "Objetos grises vendidos por:"
L["X Offset"] = "Desfase X"
L["Y Offset"] = "Desfase Y"  --Another variation in unitframes yOffset
L["You must be at a vendor."] = "Debes estar con un vendedor"

--Buffs and Debuffs
L['AURAS_DESC'] = 'Configure the aura icons that appear near the minimap.';
L['Begin a new row or column after this many auras.'] = "Empieza una nueva fila o columna después de estos auras.";
L['Consolidated Buffs'] = "Beneficios Consolidados";
L['Defines how the group is sorted.'] = "Define como se ordena el grupo.";
L['Defines the sort order of the selected sort method.'] = "Define el orden de ordenación para el método de ordenación seleccionado.";
L['Display the consolidated buffs bar.'] = "Mostrar la barra de los beneficios consolidados.";
L["Fade Threshold"] = "Umbral de Transparencia";
L['Index'] = "Índice";
L['Indicate whether buffs you cast yourself should be separated before or after.'] = "Indica si los beneficios lanzados por tí deberían estar separados antes o después.";
L['Limit the number of rows or columns.'] = "Limita el numero de filas o de columnas.";
L['Max Wraps'] = "Máximas Envolturas";
L['No Sorting'] = "No Ordenar";
L["Other's First"] = "Los de Otros Primero";
L['Seperate'] = "Separar";
L['Set the size of the individual auras.'] = "Establece el tamaño de las auras individuales.";
L['Sort Method'] = "Método de Ordenación";
L['Threshold before text changes red, goes into decimal form, and the icon will fade. Set to -1 to disable.'] = "Umbral antes de que el texto cambie a rojo, entre en forma decimal, y el icono se desvanezca. Establecer a -1 para desactivar.";
L['Time'] = "Tiempo";
L['Wrap After'] = "Envolver Después";
L['Your Auras First'] = "Tus Auras Primero";

--Chat
L["AFK"] = "Ausente" --Also used in datatexts and tooltip
L["Attempt to create URL links inside the chat."] = "Trata de crear enlaces URL dentro del chat."
L["BG"] = "CB"
L["BGL"] = "LCB"
L["BN:"] = true;
L["Chat"] = "Chat"
L["CHAT_DESC"] = "Configura los ajustes del chat para ElvUI."
L["Copy Text"] = "Copiar Texto";
L["Display the hyperlink tooltip while hovering over a hyperlink."] = "Muestra el tooltip del enlace cuando pasas el cursor sobre un enlace."
L['DND'] = "Oc"
L["Font Outline"] = "Contorno de Fuente" --Also used in UnitFrames section
L["Font"] = "Fuente"
L["G"] = "H"
L['Hide Both'] = "Ocultar Ambos"
L["Hyperlink Hover"] = "Estár Encima de Hipervínculo";
L['Invalid Target'] = "Objetivo Inválido"
L["Keyword Alert"] = "Alerta por Palabra Clave";
L['Keywords'] = "Palabras Claves";
L['Left Only'] = "Sólo el Izquierdo"
L['List of words to color in chat if found in a message. If you wish to add multiple words you must seperate the word with a comma. To search for your current name you can use %MYNAME%.\n\nExample:\n%MYNAME%, ElvUI, RBGs, Tank'] = true;
L["Number of time in seconds to scroll down to the bottom of the chat window if you are not scrolled down completely."] = "Tiempo en segundos para desplazarse al final de la ventana de chat si no se ha desplazado completamente hasta el final."
L["O"] = "O"
L["P"] = "G"
L["Panel Backdrop"] = "Fondo del Panel"
L["Panel Height"] = "Altura del Panel"
L["Panel Texture (Left)"] = "Textura del Panel Izquierdo"
L["Panel Texture (Right)"] = "Textura del Panel Derecho"
L['Panel Width'] = "Ancho del Panel"
L['PANEL_DESC'] = "Ajusta el tamaño de los paneles izquierdo y derecho. Esto afectará las ventanas de chat y las bolsas."
L["PG"] = "GG"
L["PL"] = "LG"
L["Prevent the same messages from displaying in chat more than once within this set amount of seconds, set to zero to disable."] = "Previene que los mismos mensajes se muestren más de una vez en el chat dentro de un cierto número de segundos. Establécelo a cero para desactivar."
L["R"] = "B"
L['Right Only'] = "Sólo el Derecho"
L["RL"] = "LB"
L["RW"] = "AB"
L['says'] = "dice"
L["Scroll Interval"] = "Intervalo de Desplazamiento"
L["Set the font outline."] = "Establece el contorno de fuente." --Also used in UnitFrames section
L["Short Channels"] = "Recortar Canales"
L["Shorten the channel names in chat."] = "Recorta los nombre de canal en el chat."
L['Show Both'] = "Mostrar Ambos"
L["Spam Interval"] = "Intervalo de Spam"
L["Sticky Chat"] = "Chat Pegajoso";
L["Tab Font Outline"] = "Contorno de Fuente de la Pestaña";
L["Tab Font Size"] = "Tamaño de Fuente de la Pestaña";
L["Tab Font"] = "Fuente de la Pestaña";
L['Toggle showing of the left and right chat panels.'] = "Muestra/Oculta los paneles de chat izquierdo y derecho."
L['Trade'] = "Intercambio"
L["URL Links"] = "Enlaces URL"
L["When opening the Chat Editbox to type a message having this option set means it will retain the last channel you spoke in. If this option is turned off opening the Chat Editbox should always default to the SAY channel."] = true;
L["Whisper Alert"] = "Alerta de Susurro";
L['whispers'] = "susurra"
L['yells'] = "grita"
L[ [=[Specify a filename located inside the World of Warcraft directory. Textures folder that you wish to have set as a panel background.

Please Note:
-The image size recommended is 256x128
-You must do a complete game restart after adding a file to the folder.
-The file type must be tga format.

Example: Interface\AddOns\ElvUI\media\textures\copy

Or for most users it would be easier to simply put a tga file into your WoW folder, then type the name of the file here.]=] ] = [=[Especifica un archivo ubicado en el directorio texture de World of Warcraft que deseas tener establecido como fondo de panel.

Nota:
-El tamaño de imagen recomendada es 256x128
-Debes reiniciar el juego completamente después de agregar un archivo a la carpeta.
-El archivo debe ser formato tga.

Ejemplo: Interface\AddOns\ElvUI\media\textures\copy

O también puedes simplemente colocar un archivo tga en la carpeta de WoW, y escribir aquí el nombre del archivo.]=]

--Config / Install
L["Caster DPS"] = "DPS Hechizos"
L["Chat Set"] = "Chat Configurado"
L['Choose a theme layout you wish to use for your initial setup.'] = "Elige un tema de distribución para usar en tu configuración inicial.";
L['Classic'] = "Clásico";
L["Click the button below to resize your chat frames, unitframes, and reposition your actionbars."] = "Haz clic en el botón de abajo para cambiar el tamaño de los cuadros de chat y de unidad, y reubicar tus barras de acción."
L['Config Mode:'] = "Modo de Configuración";
L["CVars Set"] = "CVars Configuradas"
L["CVars"] = "CVars"
L["ElvUI Installation"] = "Instalación de ElvUI"
L["Finished"] = "Terminado"
L["Grid Size:"] = "Tamaño de la Rejilla:"
L["Healer"] = "Sanador"
L["High Resolution"] = "Alta Resolución";
L["high"] = "alta"
L["Importance: |cff07D400High|r"] = "Importancia: |cff07D400Alta|r"
L["Importance: |cffD3CF00Medium|r"] = "Importancia: |cffD3CF00Media|r"
L["Importance: |cffFF0000Low|r"] = "Importancia: |cffFF0000Baja|r"
L["Install"] = "Instalar"
L["Installation Complete"] = "Instalación Completa"
L["Layout Set"] = "Distribución Establecida"
L["Layout"] = "Distribución"
L["Lock"] = "Bloquear"
L['Low Resolution'] = "Baja Resolución";
L["low"] = "baja"
L["Movers unlocked. Move them now and click Lock when you are done."] = "Movedores desbloqueados. Muévelos ahora y haz click en Bloquear cuando termines."
L["Physical DPS"] = "DPS Físico"
L["Please click the button below so you can setup variables and ReloadUI."] = "Haz clic en el botón de abajo para configurar variables y recargar la interfaz."
L["Please click the button below to setup your CVars."] = "Haz clic en el botón de abajo para configurar las CVars"
L["Please press the continue button to go onto the next step."] = "Presiona el botón de continuar para ir al siguiente paso"
L["Resolution Style Set"] = "Estilo de Resolución Establecido"
L["Resolution"] = "Resolución"
L["Run the installation process."] = "Ejecutar el proceso de instalación"
L["Setup Chat"] = "Configurar Chat"
L["Setup CVars"] = "Configurar CVars"
L["Skip Process"] = "Saltar Proceso"
L["Sticky Frames"] = "Cuadros Adhesivos"
L["Tank"] = "Tanque"
L["The chat windows function the same as Blizzard standard chat windows, you can right click the tabs and drag them around, rename, etc. Please click the button below to setup your chat windows."] = "Las ventanas de chat funcionan igual que sus contrapartes estándar de Blizzard. Puedes hacer clic derecho en las pestañas y arrastrarlas, cambiarles el nombre, etc. Haz clic en el botón de abajo para configurar las ventanas de chat."
L["The in-game configuration menu can be accesses by typing the /ec command or by clicking the 'C' button on the minimap. Press the button below if you wish to skip the installation process."] = "El menú de configuración puede ser accedido mediante el comando /ec o haciendo clic en el botón 'C' del minimapa. Presiona el botón de abajo si deseas saltarte la instalación."
L['Theme Set'] = "Establecer Tema";
L['Theme Setup'] = "Configurar Tema";
L["This install process will help you learn some of the features in ElvUI has to offer and also prepare your user interface for usage."] = "El proceso de instalación te ayudará a aprender algunas de las características de ElvUI y preparará la interfaz para su uso."
L["This is completely optional."] = "Esto es completamente opcional."
L["This part of the installation process sets up your chat windows names, positions and colors."] = "Esta parte de la instalación configura los nombres, posiciones y colores de las ventanas de chat."
L["This part of the installation process sets up your World of Warcraft default options it is recommended you should do this step for everything to behave properly."] = "Esta parte de la instalación configura las opciones predeterminadas de World of Warcraft. Se recomienda hacer este paso para que todo funciones apropiadamente."
L["This resolution doesn't require that you change settings for the UI to fit on your screen."] = "Esta resolución no necesita que cambies los ajustes para que quepa la interfaz en tu pantalla."
L["This resolution requires that you change some settings to get everything to fit on your screen."] = "Esta resolución requiere que cambies algunos ajustes para que todo quepa en tu pantalla."
L["This will change the layout of your unitframes, raidframes, and datatexts."] = "Esto cambiará la distribución de tus cuadros de unidad, de banda y textos de datos."
L["Toggle Anchors"] = "Mostrar/Ocultar Fijadores"
L['Toggle Configuration'] = "Mostrar/Ocultar Configuración";
L["Unlock various elements of the UI to be repositioned."] = "Desbloquea varios elementos de la interfaz para ser reubicados."
L["Using the healer layout it is highly recommended you download the addon Clique to work side by side with ElvUI."] = "Usando la distribución de sanador es altamente recomendable que descargues el addon Clique para trabajar junto con ElvUI.";
L["Version"] = "Versión"
L["You are now finished with the installation process. If you are in need of technical support please visit us at http://www.tukui.org."] = "Ya has terminado con el proceso de instalación. Si necesitas ayuda o soporte técnico por favor visitanos en http://www.tukui.org.";
L['You can always change fonts and colors of any element of elvui from the in-game configuration.'] = "Siempre puedes cambiar las letras y colores de cualquier elemento de ElvUI desde la configuración.";
L["You can now choose what layout you wish to use based on your combat role."] = "Ahora puedes elegir que distribución quieres basándote en tu rol de combate."
L["You may need to further alter these settings depending how low you resolution is."] = "Puede que necesites cambiar estos ajutes dependiendo de qué tan baja sea tu resolución."
L["Your current resolution is %s, this is considered a %s resolution."] = "Tu resolución actual es %s, esto se considera una resolución %s."

--Credits
L["Coding:"] = "Codificación:"
L["Credits"] = "Créditos"
L["Donations:"] = "Donativos:"
L["ELVUI_CREDITS"] = "Quiero dar un agradecimiento especial a las siguientes personas por ayudar a probar y codificar este addon y también a quienes me ayudaron con donativos. Nota: Para los donativos sólo muestro los nombres de quienes me enviaron un mensaje en el foro. Si tu nombre no aparece y quieres que lo agregue mándame un mensaje."
L["Testing:"] = "Pruebas:"

--DataTexts
L["(Hold Shift) Memory Usage"] = "(Mantén Shift) Uso de Memoria"
L["24-Hour Time"] = "Tiempo de 24 horas"
L['Always Display'] = "Mostrar Siempre";
L["AP"] = "PA"
L["AVD: "] = "EVA: "
L["Avoidance Breakdown"] = "Desglose de Evasión"
L["Bandwidth"] = "Ancho de Banda"
L['Bases Assaulted'] = "Bases Asaltadas";
L['Bases Defended'] = "Bases Defendidas";
L['Battleground datatexts temporarily hidden, to show type /bgstats or right click the "C" icon near the minimap.'] = "Textos de datos de los campos de batalla temporalmente ocultos, para mostrarlos escribe /bgstats o click derecho en "C" donde el minimapa.";
L['Battleground datatexts will now show again if you are inside a battleground.'] = "Textos de datos de los campos de batalla visibles de nuevo si estás en un campo de batalla.";
L['Battleground Texts'] = "Textos de los Campos de Batalla";
L['Carts Controlled'] = "Vehiculos Controlados";
L['Change settings for the display of the location text that is on the minimap.'] = "Cambia la configuración para mostrar el texto de localización que está en el minimapa.";
L["Character: "] = "Personaje: "
L["Chest"] = "Pecho"
L["DataTexts"] = "Textos de Datos"
L["DATATEXT_DESC"] = "Configura el despliegue en pantalla de los textos de datos."
L["Deficit:"] = "Déficit:"
L['Demolishers Destroyed'] = "Demoledores Destruidos";
L['Display minimap panels below the minimap, used for datatexts.'] = "Muestra los paneles del minimapa debajo del minimapa, usado para los textos de datos.";
L['Download'] = "Descarga"
L["DPS"] = "DPS"
L["Earned:"] = "Ganada:"
L["Feet"] = "Pies"
L['Flags Captured'] = "Bandera Capturada";
L['Flags Returned'] = "Bandera Recuperada";
L["Friends List"] = "Lista de Amigos"
L['Gates Destroyed'] = "Puertas Destruídas";
L['Graveyards Assaulted'] = "Cementerios Asaltados";
L['Graveyards Defended'] = "Cementerios Defendidos";
L["Hands"] = "Manos"
L["Head"] = "Cabeza"
L["Hit"] = "Golpe"
L["Home Latency:"] = "Latencia Local:"
L["HP"] = "Salud";
L["HPS"] = "VPS"
L["If not set to true then the server time will be displayed instead."] = "Se no se activa entonces se mostrará la hora del servidor."
L["left"] = "Izquierda"
L["LeftChatDataPanel"] = "Panel de Chat Izquierdo"
L["LeftMiniPanel"] = "Panel de Minimapa Izquierdo"
L["Legs"] = "Piernas"
L["Local Time"] = "Hora Local"
L['Location Text'] = "Textos de Localización";
L["lvl"] = "Niv"
L["Main Hand"] = "Mano Derecha"
L["middle"] = "Medio"
L['Minimap Mouseover'] = "Ratón por encima del Minimapa";
L['Minimap Panels'] = "Paneles del Minimapa";
L['Mitigation By Level: '] = "Mitigación Por Nivel: "
L['No Guild'] = "Sin Hermandad";
L["Offhand"] = "Mano Izquierda"
L['Orb Possessions'] = "Orbes en Posesión";
L["Panels"] = "Paneles"
L["Profit:"] = "Ganancia:"
L["right"] = "Derecha"
L["RightChatDataPanel"] = "Panel de Chat Derecho"
L["RightMiniPanel"] = "Panel de Minimapa Derecho"
L["Saved Raid(s)"] = "Banda(s) Guardada(s)"
L["Server: "] = "Servidor: "
L["Session:"] = "Sesión:"
L["Shoulder"] = "Hombros"
L['Show BG Texts'] = "Mostrar Textos de CB";
L["SP"] = "PH"
L["Spent:"] = "Gastada:"
L['Stats For:'] = "Estadísticas para:";
L["Toggle 24-hour mode for the time datatext."] = "Ver formato de 24 horas para el texto de datos de tiempo."
L["Total CPU:"] = "CPU Total:"
L["Total Memory:"] = "Memoria Total:"
L["Total: "] = "Total: "
L['Towers Assaulted'] = "Torres Asaltadas";
L['Towers Defended'] = "Torres Defendidas";
L["Unhittable:"] = "Imbatible:"
L['Victory Points'] = "Puntos de Victoria";
L["Waist"] = "Cintura"
L['When inside a battleground display personal scoreboard information on the main datatext bars.'] = "Cuando estás dentro de un campo de batalla muestra la puntuación personal en las barras de texto principales.";
L["Wrist"] = "Muñeca"

--DebugTools
L["%s: %s tried to call the protected function '%s'."] = "%s: %s intento llamar a la función protegida '%s'.";
L["INCOMPATIBLE_ADDON"] = "El addon %s no es compatible con el módulo %s de ElvUI. Por favor deshabilita el addon o módulo incompatibles."
L["No locals to dump"] = "No hay locales para volcar";
L['|cFFE30000Lua error recieved. You can view the error message when you exit combat.'] = "|cFFE30000Error de Lua recivido. Podrás ver el error cuando salgas de combate.";

--General
L["Accept Invites"] = "Aceptar Invitaciones"
L['Adjust the position of the threat bar to either the left or right datatext panels.'] = "Ajusta la posición de la barra de amenaza a la izquierda o derecha de los paneles de texto de datos.";
L['Adjust the size of the minimap.'] = "Ajusta el tamaño del minimapa."
L["Announce Interrupts"] = "Anunciar Interrupciones"
L["Announce when you interrupt a spell to the specified chat channel."] = "Anunciar cuando interrumpas un hechizo en el canal especificado."
L['Auto Greed/DE'] = "Codicia/Desencantar Automático";
L["Auto Repair"] = "Reparación Automática"
L["Auto Scale"] = "Escalado Automático"
L["Automatically accept invites from guild/friends."] = "Aceptar de forma automática invitaciones de la hermandad/amigos."
L["Automatically repair using the following method when visiting a merchant."] = "Repara de forma automática usando el siguiente método cuando visites un comerciante."
L["Automatically scale the User Interface based on your screen resolution"] = "Escala de forma automática la interfaz de usuario dependiendo de la resolución de pantalla"
L['Automatically select greed or disenchant (when available) on green quality items. This will only work if you are the max level.'] = "Tira codicia o desencanta (si se puede) automaticamente para los objetos verdes. Esto solo funciona si ya tienes el nivel máximo.";
L["Automatically vendor gray items when visiting a vendor."] = "Vender automáticamente los objetos grises al visitar al vendedor.";
L["Chat Bubbles"] = "Burbujas de Chat";
L["Controls what the transparency of the worldmap will be set to when you are moving."] = "Controla la tranparencia del mapa mundial cuando te mueves."
L["Disband Group"] = "Disolver Grupo"
L['Display emotion icons in chat.'] = "Muestra emoticonos en el chat.";
L["Don't scale the large world map to block out sides of the screen."] = "No escalar el mapa del mundo grande hasta los bordes de la pantalla.";
L["ELVUI_DESC"] = "ElvUI es un addon de reemplazo de interfaz completo para World of Warcraft."
L['Emotion Icons'] = "Emoticonos";
L["Enable/Disable the loot frame."] = "Activa/Desactiva el cuadro de despojo.";
L["Enable/Disable the loot roll frame."] = "Activa/Desactiva el cuadro de despojo por sorteo.";
L['Enable/Disable the minimap. |cffFF0000Warning: This will prevent you from seeing the consolidated buffs bar, and prevent you from seeing the minimap datatexts.|r'] = "Activa/Desactiva el minimapa. |cffFF0000Atención: Esto evita que veas la barra de los beneficios consolidados, y evita que veas los textos de datos del minimapa.|r";
L["General"] = "General"
L['Left'] = "Izquierda"
L["Log Taints"] = "Contaminación del Log";
L["Login Message"] = "Mensaje de inicio"
L["LOGIN_MSG"] = "Welcome to %sElvUI|r version %s%s|r, type /ec to access the in-game configuration menu. If you are in need of technical support you can visit us at http://www.tukui.org"
L["Loot Roll"] = "Cuadro de Botín"
L["Loot"] = "Botín"
L["Map Alpha While Moving"] = "Mapa Transparente Mientras se Mueve";
L["Reset all frames to their original positions."] = "Coloca todos los cuadros en sus posiciones originales"
L["Reset Anchors"] = "Reestablecer fijadores"
L['Right'] = "Derecha"
L["Send ADDON_ACTION_BLOCKED errors to the Lua Error frame. These errors are less important in most cases and will not effect your game performance. Also a lot of these errors cannot be fixed. Please only report these errors if you notice a Defect in gameplay."] = true;
L["Skin the blizzard chat bubbles."] = "Burbujas de Chat con la Piel de Blizzard.";
L["Tiny Map"] = "Mapa Pequeño";
L['Toggle Tutorials'] = "Mostrar/Ocultar Tutoriales";
L["Welcome to ElvUI version %s!"] = "Bienvenido(a) a ElvUI versión %s!"

--Media
L["Backdrop color of transparent frames"] = "Color de fondo de los cuadros transparentes."
L["Backdrop Color"] = "Color de Fondo"
L["Backdrop Faded Color"] = "Color Atenuado de Fondo"
L["Border Color"] = "Color de Borde"
L["Color some texts use."] = "Color que usan algunos textos."
L["Colors"] = "Colores" --Also used in UnitFrames
L["CombatText Font"] = "Fuente del Texto de Combate"
L["Default Font"] = "Fuente Predeterminada"
L["Font Size"] = "Tamaño de la Fuente" --Also used in UnitFrames
L["Fonts"] = "Fuentes"
L["Main backdrop color of the UI."] = "Color principal de fondo para la interfaz."
L["Main border color of the UI."] = "Color principal de borde para la interfaz."
L["Media"] = "Medios"
L["Primary Texture"] = "Textura Primaria"
L["Secondary Texture"] = "Textura Secundaria"
L["Set the font size for everything in UI. Note: This doesn't effect somethings that have their own seperate options (UnitFrame Font, Datatext Font, ect..)"] = "Establece el tamaño de la fuente para la interfaz. Nota: Esto no afecta elementos que tengan sus propias opciones (Cuadros de Unidad, Textos de Datos, etc.)"
L["Textures"] = "Texturas"
L["The font that combat text will use. |cffFF0000WARNING: This requires a game restart or re-log for this change to take effect.|r"] = "La fuente que el texto de combate usará. |cffFF0000ADVERTENCIA: Este requiere un reinicio del juego o salir y entrar nuevamente para que este cambio surta efecto.|r"
L["The font that the core of the UI will use."] = "La fuente que el núcleo de la interfaz usará."
L["The texture that will be used mainly for statusbars."] = "La textura que se usará principalmente para barras de estado."
L["This texture will get used on objects like chat windows and dropdown menus."] = "Esta textura se usará en objetos como las ventanas de chat y menús desplegables."
L["Value Color"] = "Color de Valor"

--Misc
L["Can't Roll"] = "No puede echar suertes"
L["Empty Slot"] = "Espacio vacío"
L["Enable"] = "Habilitar" --Doesn't fit a section since it's used a lot of places
L["Experience"] = "Experiencia"
L["Fishy Loot"] = "Botín Sospechoso"
L['Left Chat'] = "Chat Izquierdo";
L["Left Click:"] = "Click Izquierdo"; --layout\layout.lua
L["Raid Menu"] = "Menú de Banda"
L["Remaining:"] = "Restante"
L["Rested:"] = "Descansado:"
L['Right Chat'] = "Chat Derecho";
L["Right Click:"] = "Click Derecho"; --layout\layout.lua
L["Toggle Chat Frame"] = "Mostrar/Ocultar Cuadro de Chat"; --layout\layout.lua
L["XP:"] = "XP:"
L["You don't have enough money to repair."] = "No tienes suficiente dinero para reparaciones."
L["You don't have permission to mark targets."] = "No tienes permiso para marcar blancos."
L["Your items have been repaired for: "] = "Tus objetos han sido reparados por:"
L["Your items have been repaired using guild bank funds for: "] = "Tus objetos han sido reparados con fondos del banco de banda por:"
L["Your version of ElvUI is out of date. You can download the latest version from http://www.tukui.org"] = "Tu versión de ElvUI no está actualizada. Puedes descargar la última versión de http://www.tukui.org" --core\core.lua

--NamePlates
L["Add Name"] = "Añadir Nombre";
L["All"] = "Todo";
L["Auras"] = "Auras";
L["Bad Color"] = "Mal Color";
L["Bad Scale"] = "Mala Escala";
L["Bad Transition Color"] = "Mal Color de Transición";
L["Castbar Height"] = "Altura de la Barra de Lanzamiento";
L['Color the border of the nameplate yellow when it reaches the threshold point on these types of frames.'] = "Colorea en amarillo el borde de la placa de nombre cuando alcanza el punto de umbral es esos tipos de cuadros.";
L['Color the border of the nameplate yellow when it reaches this point, it will be colored red when it reaches half this value.'] = "Colorea en amarillo el borde de la placa de nombre cuando alcanza este punto, se coloreará en rojo cuando alcance la mitad de este valor.";
L["Color the nameplate's healthbar by your current threat, Example: good threat color is used if your a tank when you have threat, opposite for DPS."] = "Colorea la barra de salud de la placa de nombre en base a tu amenaza actual, Ejemplo: se usa un color bueno de amenaza si tu eres tanque cuando tienes la amenaza, al contrario si eres DPS.";
L["Combat Toggle"] = "Alternar en Combate";
L["Controls the height of the nameplate"] = "Controla la altura de la placa de nombre";
L["Controls the height of the nameplate's castbar"] = "Controla la altura de la barra de lanzamiendo de la placa de nombre";
L["Controls the width of the nameplate"] = "Controla la anchura de la placa de nombre";
L["Custom Color"] = "Color personalizado";
L["Custom Scale"] = "Escala personalizada";
L["Disable threat coloring for this plate and use the custom color."] = "Desactiva el coloreado por amenaza para esta placa y usa un color personalizado.";
L["Discipline"] = "Disciplina";
L["Display a healer icon over known healers inside battlegrounds."] = "Muestra un icono de sanador sobre los sanadores conocidos dentro de los campos de batalla";
L["Display level text on nameplate for nameplates that belong to units that aren't your level."] = "Muestra el nivel en la placa de nombre para aquellas unidades que tienen un nivel distinto al tuyo.";
L["Display your personal debuffs over the nameplate."] = "Muestra sus perjuicios personales encima de la placa de nombre.";
L["Enemy"] = "Enemigo";
L["Enhance Threat"] = "Amenaza Mejorada";
L["Filter already exists!"] = "¡El filtro ya existe!";
L["Filters"] = "Filtros"; --Also used in UnitFrames
L["Friendly NPC"] = "PNJ Amistoso";
L["Friendly Player"] = "Jugador Amistoso";
L["Good Color"] = "Buen Color";
L["Good Scale"] = "Buena Escala";
L["Good Transition Color"] = "Buen Color de Transición";
L["Healer Icon"] = "Icono de Sanador";
L["Hide"] = "Ocultar"; --Also used in DataTexts
L["Holy"] = "Sagrado";
L['Low Health Threshold'] = "Umbral de Baja Salud";
L['Low Health Warning'] = "Alerta de Baja Salud";
L["NamePlates"] = "Placas de Nombre";
L["NAMEPLATE_DESC"] = "Modifica las opciones de la placa de nombre";
L["Personal Debuffs"] = "Perjuicios Personales";
L['Players'] = "Jugadores";
L["Prevent any nameplate with this unit name from showing."] = "Previene que se muestre cualquier placa de nombre con este nombre de unidad.";
L["Remove Name"] = "Eliminar Nombre";
L["Restoration"] = "Restauración";
L["Select a filter to use. These are imported from the unitframe aura filter."] = "Selecciona el filtro a usar. Esos son importados desde el filtro del cuadro de unidad."; --This one is not visible in the GUI since desc is not used on select groups. Keep for dropdown based menus?
L["Set the scale of the nameplate."] = "Establece la escala de la placa de nombre";
L["This color is displayed when gaining/losing threat, for a tank it would be displayed when gaining threat, for a dps/healer it would be displayed when losing threat"] = "Este color se muestra cuando ganas/pierdes amenaza, para un tanque se mostrará cuando gane amenaza, para un dps/sanador lo hará cuando la pierda";
L["This color is displayed when gaining/losing threat, for a tank it would be displayed when losing threat, for a dps/healer it would be displayed when gaining threat"] = "Este color se muestra cuando ganas/pierdes amenaza, para un tanque se mostrará cuando pierda amenaza, para un dps/sanador lo hará cuando la gane";
L["This is displayed when you don't have threat as a tank, if you do have threat it is displayed as a DPS/Healer"] = "Esto se muestra cuando no tienes la amenaza como tanque, si eres dps/sanador se muestra cuando tienes la amenaza";
L["This is displayed when you have threat as a tank, if you don't have threat it is displayed as a DPS/Healer"] = "Esto se muestra cuando tienes la amenaza como tanque, si eres dps/sanador se muestra cuando no tienes la amenaza";
L["Threat"] = "Amenaza";
L["Toggles the nameplates off when not in combat."] = "Oculta las placas de nombre cuando no estás en combate.";
L["Use this filter."] = "Usar este filtro.";
L["You can't remove a default name from the filter, disabling the name."] = "No puedes eliminar un nombre por defecto del filtro, inutilizando el nombre.";

--Skins
L["Achievement Frame"] = "Cuadro de Logros";
L['Alert Frames'] = "Cuadro de Alertas";
L["Archaeology Frame"] = "Cuadro de Arqueología";
L["Auction Frame"] = "Cuadro de Subastas";
L["Barbershop Frame"] = "Cuadro de Barbería";
L["BG Map"] = "Mapa de CB";
L["BG Score"] = "Puntuación de CB";
L["Calendar Frame"] = "Cuadro de Calendario";
L["Character Frame"] = "Cuadro de Personaje";
L["Debug Tools"] = "Herramientas de Depuración";
L["Dressing Room"] = "Probador";
L["Encounter Journal"] = "Diario de Encuentros";
L["Friends"] = "Amigos";
L["Glyph Frame"] = "Cuadro de Glifos";
L["Gossip Frame"] = "Cuadro de Actualidad";
L["Greeting Frame"] = "Cuadro de Bienvenida";
L["Guild Bank"] = "Banco de Hermandad";
L["Guild Control Frame"] = "Cuadro de Control de Hermandad";
L["Guild Frame"] = "Cuadro de Hermandad";
L["Guild Registrar"] = "Registrar Hermandad";
L["Help Frame"] = "Cuadro de Ayuda";
L["Inspect Frame"] = "Cuadro de Inspección";
L["KeyBinding Frame"] = "Cuadro de Asignación de Teclas";
L["LF Guild Frame"] = "Cuadro de Busqueda de Hermandad";
L["LFG Frame"] = "Cuadro de Busqueda de Grupo";
L["Loot Frames"] = "Cuadro de Despojo";
L["Macro Frame"] = "Cuadro de Macros";
L["Mail Frame"] = "Cuadro de Correo";
L["Merchant Frame"] = "Cuadro de Mercader";
L["Misc Frames"] = "Cuadros de Miscelanea";
L["Mounts & Pets"] = "Monturas y Macotas";
L["Non-Raid Frame"] = "Cuadro de no-Banda";
L["Pet Battle"] = "Combate de Macotas";
L["Petition Frame"] = "Cuadro de Petición";
L["PvP Frames"] = "Cuadros JcJ";
L["Quest Frames"] = "Cuadros de Misión";
L["Raid Frame"] = "Cuadros de Banda";
L["Reforge Frame"] = "Cuadro de Reforje";
L["Skins"] = "Pieles";
L["SKINS_DESC"] = "Adjust Skin settings."
L["Socket Frame"] = "Cuadro de Incrustación";
L["Spellbook"] = "Libro de Hechizos";
L["Stable"] = "Establo";
L["Tabard Frame"] = "Cuadro de Tabardos";
L["Talent Frame"] = "Cuadro de Talentos";
L["Taxi Frame"] = "Cuadro de Viaje";
L["Time Manager"] = "Administrador de Tiempo";
L["TOGGLESKIN_DESC"] = "Enable/Disable this skin."
L["Trade Frame"] = "Cuadro de Comercio";
L["TradeSkill Frame"] = "Cuadro de Comercio de Habilidades";
L["Trainer Frame"] = "Cuadro de Entrenador";
L["World Map"] = "Mapa del Mundo";

--Static Popups
L["A setting you have changed will change an option for this character only. This setting that you have changed will be uneffected by changing user profiles. Changing this setting requires that you reload your User Interface."] = "La opción que has cambiado se aplicará solo para este personaje. Esta opción se verá inalterada al cambiar el perfil de usuario. Cambiar esta opción requiere que recargues tu Interfaz de Usuario.";
L["Are you sure you want to delete all your gray items?"] = "¿Estas seguro que quieres eliminar todos tus objetos grises?";
L["Are you sure you want to disband the group?"] = "¿Estas seguro que quieres deshacer el grupo?";
L["Are you sure you want to reset every mover back to it's default position?"] = "¿Estas seguro que quieres resetear cada cuadro movil a su posición por defecto?";
L["Can't buy anymore slots!"] = "¡No puedes comprar más huecos!";
L["Hover your mouse over any actionbutton or spellbook button to bind it. Press the escape key or right click to clear the current actionbutton's keybinding."] = "Pasa tu ratón por encima de un botón de acción o de un botón del libro de hechizos para ligarlo. Pulsa escape o botón derecho para limpiar la asignación actual del botón de acción.";
L["One or more of the changes you have made require a ReloadUI."] = "Uno o más de los cambios que has hecho requieren una recarga de la interfaz.";
L["One or more of the changes you have made will effect all characters using this addon. You will have to reload the user interface to see the changes you have made."] = "Uno o más de los cambios que has hecho afectaran a todos los personajes que usen este addon. Tendrás que recargar la intefaz de usuario para ver el cambio que has realizado.";
L["Save"] = "Guardar";
L["You have changed your UIScale, however you still have the AutoScale option enabled in ElvUI. Press accept if you would like to disable the Auto Scale option."] = "Has cambiado la escala de tu interfaz, sin embargo aun tienes el AutoEscalado activado en ElvUI. Pulsa aceptar si te gustaría desactivar el AutoEscalado.";
L["You must purchase a bank slot first!"] = "¡Debes comprar un hueco del banco primero!";

--Tooltip
L["Anchor Mode"] = "Modo de anclaje";
L["Anchor"] = "Anclaje";
L["Combat Hide"] = "Ocultar en Combate";
L["Count"] = "Contador";
L["Cursor"] = "Cursor";
L["Don't display the tooltip when mousing over a unitframe."] = "No mostrar la descripción emergente cuando pasas el ratón sobre un cuadro de unidad.";
L["Hide tooltip while in combat."] = "Oculta la descripción emergente mientras estás en combate.";
L["Set the type of anchor mode the tooltip should use."] = "Establece el tipo de anclaje que debería usar la descripción emergente.";
L["Smart"] = "Pequeño";
L["Targeted By:"] = "Objetivo De:";
L["Tooltip"] = "Descripción Emergente";
L["TOOLTIP_DESC"] = "Configuración para las Descipciones Emergentes."
L["UF Hide"] = "Ocultar en Cuadros de Unidad";
L["When in a raid group display if anyone in your raid is targetting the current tooltip unit."] = "Cuando estas en una banda mostrar si alguien en tu banda tiene marcado como objetivo a la unidad actual de la descripción emergente.";
L["Who's targetting who?"] = "¿Quién tiene como objetivo a quién?";

--Tutorials
L['A raid marker feature is available by pressing Escape -> Keybinds scroll to the bottom under ElvUI and setting a keybind for the raid marker.'] = "La opción de marcador de banda está disponible pulsando Escape -> Asignar teclas -> Recorrer hacia abajo hasta ElvUI y establecer la tecla para el marcador de banda.";
L['ElvUI has a dual spec feature which allows you to load different profiles based on your current spec on the fly. You can enable this from the profiles tab.'] = "ElvUI tiene la posibilidad de cargar diferentes perfiles automaticamente al cambiar de especialidad de talentos. Puedes activar esta función en la pestaña de perfiles.";
L['For technical support visit us at http://www.tukui.org.'] = "Para soporte técnico visitanos en http://www.tukui.org.";
L['If you accidently remove a chat frame you can always go the in-game configuration menu, press install, go to the chat portion and reset them.'] = "Si eliminas un cuadro de chat accidentalmente, siempre puedes ir a la configuración, pulsar instalar, ir a la parte del chat, y restaurarlo.";
L['If you are experiencing issues with ElvUI try disabling all your addons except ElvUI, remember ElvUI is a full UI replacement addon, you cannot run two addons that do the same thing.'] = "Si has experimentado errores con ElvUI prueba a desactivar todos tus addons excepto ElvUI, recuerda que ElvUI es un addon que remplaza por completo la interfaz, no puedes tener más de un addon que haga lo mismo.";
L['If you need to reset the gold datatext type /resetgold.'] = "Si necesitas borrar el cuadro de texto del oro escribe /resetgold.";
L['The buff panel to the right of minimap is a list of your consolidated buffs. You can disable it in Buffs and Debuffs options of ElvUI.'] = "El panel de beneficios a la derecha del minimap es una lista de los beneficios consolidados. Puedes desactivarlo en la pestaña de Beneficios y Perjuicios de ElvUI.";
L['The focus unit can be set by typing /focus when you are targetting the unit you want to focus. It is recommended you make a macro to do this.'] = "El foco puede establecerse escribiendo /focus cuando tienes seleccionado al objetivo que quieres hacer foco. Es recomendable que hagas una macro para esto.";
L['To move abilities on the actionbars by default hold shift + drag. You can change the modifier key from the actionbar options menu.'] = "Para mover habilidades a las barras de acción mantener shift + arrastrar. Puedes cambiar la tecla de modificación desde el menú de opciones de la barra de acción.";
L['To setup which channels appear in which chat frame, right click the chat tab and go to settings.'] = "Para configurar que canales aparecen en el chat, botón derecho el la pestaña del chat y ir a opciones.";
L['Using the /farmmode <size> command will spawn a larger minimap on your screen that can be moved around, very useful when farming.'] = "Usando el comando /farmode <tamaño> hace aparecer un minimapa más grande en su pantaña que puede moverse, muy util para recolectar.";
L['You can access copy chat and chat menu functions by mouse over the top right corner of chat panel and left/right click on the button that will appear.'] = "Puedes acceder a copiar y a las opciones del chat pasando el raton sobre la esquina derecha del panel del chat y haciendo click en el boton que aparece.";
L['You can see someones average item level of their gear by holding shift and mousing over them. It should appear inside the tooltip.'] = "Puedes ver la media de nivel de objeto de un objetivo manteniendo pulsado shift mientras pasas el raton por encima de el. El iNvl aparecerá en el tooltip.";
L['You can set your keybinds quickly by typing /kb.'] = "Puedes establecer tus atajos rapidamente escribiendo /kb.";
L['You can toggle the microbar by using your middle mouse button on the minimap you can also accomplish this by enabling the actual microbar located in the actionbar settings.'] = "Puedes acceder a la microbarra usando tu botón central del raton sobre el minimapa. Tambien puedes dejarla permanentemente acivada en las opciones de la microbarra.";
L['You can use the /resetui command to reset all of your movers. You can also use the command to reset a specific mover, /resetui <mover name>.\nExample: /resetui Player Frame'] = "Puedes usar el commando /resetui para restablecer todos tus cuadros moviles. También puedes usar el comando para resetear alguno específico, /resetui <mover>. Por ejemplo: /resetui Player Frame";

--UnitFrames
L["%s frame(s) has a conflicting anchor point, please change either the buff or debuff anchor point so they are not attached to each other. Forcing the debuffs to be attached to the main unitframe until fixed."] = "El cuadro(s) %s tiene un punto de anclaje en conflicto, por favor cambia cada uno de los puntos de anclaje de los beneficios y perjuicios para que no esten adjuntos entre ellos. Forzando a los perjuicios para que esten adjuntos al cuadro de unidad principal hasta que se corrija.";
L['Above'] = "Encima";
L["Add a spell to the filter."] = "Añade un hechizo al filtro.";
L["Add Spell"] = "Añadir Hechizo";
L["Add SpellID"] = "Añadir ID de Hechizo";
L['Allow displaying of auras that are considered consolidated by Blizzard.'] = "Permitir mostrar auras que son consideradas consolidadas por Blizzard.";
L['Allow displaying of auras that do not have a duration.'] = "Permitir mostrar auras que no tienen duración";
L["An X offset (in pixels) to be used when anchoring new frames."] = "Un desplazamiento X (en píxeles) para ser usado cuando se anclan nuevos cuadros.";
L["An Y offset (in pixels) to be used when anchoring new frames."] = "Un desplazamiento Y (en píxeles) para ser usado cuando se anclan nuevos cuadros.";;
L["Any Unit"] = "Cualquier Unidad";
L["Arena Frames"] = "Cuadros de Arena";
L["Assist Frames"] = "Cuadros de Asistencia";
L['Assist Target'] = "Asistir a Objetivo";
L["Attach To"] = "Adjuntar a";
L['Aura Bars'] = "Barra de Auras";
L["Bad"] = "Mal";
L["Bars will transition smoothly."] = "Las barras haran las transiciones suavemente.";
L['Bars'] = "Barras"; --Also used in ExpRepBar tooltip
L['Below'] = "Debajo";
L["Blacklist"] = "Lista Negra";
L["Boss Frames"] = "Cuadro de Jefe";
L["Buff Indicator"] = "Indicador de Beneficio";
L["Buffs"] = "Beneficios";
L["Camera Distance Scale"] = "Escala de la Distancia de la Cámara";
L["Castbar"] = "Barra de Lanzamiento";
L["Class Backdrop"] = "Fondo de Clase";
L["Class Health"] = "Salud de Clase";
L["Class Power"] = "Poder de Clase";
L["Classbar"] = "Barra de Clase";
L["Color health by amount remaining."] = "Color de salud por la cantidad restante.";
L["Color health by classcolor or reaction."] = "Color de salud por el color de clase o reacción.";
L["Color Icons"] = "Iconos de Colores";
L["Color power by classcolor or reaction."] = "Color de poder por el color de clase o reacción.";
L["Color the health backdrop by class or reaction."] = "Color de fondo de salud por el color de clase o reacción.";
L["Color the icon to their set color in the filters section, otherwise use the icon texture."] = "Color del icono por el color establecido en la sección de filtros, de otra manera usa la textura del icono.";
L["Color the unit healthbar if there is a debuff that can be dispelled by you."] = "Color de la barra de salud si hay un perjuicio que puede ser disipado por tí.";
L["Color"] = "Color";
L["Column Point"] = "Punto de Columna";
L["Column Spacing"] = "Espaciado de Columna";
L["Combat Fade"] = "Desvanecer en Combate";
L["Combobar"] = "Barra de Combo";
L["Copy From"] = "Copiar Desde";
L['Create a custom fontstring. Once you enter a name you will be able to select it from the elements dropdown list.'] = "Crear una cadena de fuentes personalizada. Una vez que introduzcas un nombre podras seleccionarlo en la lista despleglable.";
L["Create a filter, once created a filter can be set inside the buffs/debuffs section of each unit."] = "Crea un filtro, una vez creado podrás establecerlo dentro de la sección beneficios/perjuicios de cada unidad.";
L["Create Filter"] = "Crear Filtro";
L['Current - Max | Percent'] = "Actual - Máximo | Porcentaje";
L["Current - Max"] = "Actual - Máximo";
L["Current - Percent"] = "Actual - Porcentaje";
L["Current / Max"] = "Actual / Máximo";
L["Current"] = "Actual";
L["Custom Health Backdrop"] = "Fondo de Salud Personalizado";
L['Custom Texts'] = "Texto Personalizado";
L["Debuff Highlighting"] = "Resaltado de Perjuicio";
L["Debuffs"] = "Perjuicios";
L["Deficit"] = "Déficit"
L["Delete a created filter, you cannot delete pre-existing filters, only custom ones."] = "Borra el filtro creado, no puedes borrar filtro pre-existentes, solo los personalizados.";
L["Delete Filter"] = "Borrar Filtro";
L["Direction the health bar moves when gaining/losing health."] = "La dirección de la barra de salud se mueve cuando ganas/pierdes salud.";
L["Disable Blizzard"] = "Desactivar Blizzard";
L['Disabled'] = "Desactivado";
L["Disables the blizzard party/raid frames."] = "Desactiva el cuadro de grupo/banda de Blizzard";
L["Disconnected"] = "Desconectado";
L["Display a spark texture at the end of the castbar statusbar to help show the differance between castbar and backdrop."] = "Muestra una textura al final de la barra de lanzamiento/estado para ayudar a diferenciar entre la barra de lanzamiento y el fondo.";
L['Display Frames'] = "Mostrar Cuadros";
L["Display Player"] = "Mostrar Jugador";
L["Display Target"] = "Mostrar Objetivo";
L['Display icon on arena frame indicating the units talent specialization or the units faction if inside a battleground.'] = "Mostrar un icono en el cuadro de arena indicando las especializaciones de talento o la facción si es en un campo de batalla.";
L["Display the rested icon on the unitframe."] = "Muestra el icono de alimentado en el cuadro de unidad.";
L["Display the target of the cast on the castbar."] = "Muestra el objetivo en la barra de lanzamiento.";
L["Display tick marks on the castbar for channelled spells. This will adjust automatically for spells like Drain Soul and add additional ticks based on haste."] = "Muestra las marcas de cada tick en la barra de lanzamiento para los hechizos canalizados. Esto se ajustara automaticamente en base al hechizo y la celeridad.";
L["Down"] = "Abajo";
L['Enemy Aura Type'] = "Tipo de Aura Enemiga";
L["Fade the unitframe when out of combat, not casting, no target exists."] = "Desvanecer el cuadro de unidad cuando está fuera de combate, sin lanzar, o sin objetivo.";
L["Fill"] = "Llenar";
L["Filled"] = "Llenado";
L["Filter Type"] = "Tipo de Filtro";
L["Focus Frame"] = "Cuadro de Foco";
L["FocusTarget Frame"] = "Cuadro de Objetivo del Foco";
L['Force the frames to show, they will act as if they are the player frame.'] = "Forzar a mostrar los cuadros, esto funcionará si es el cuadro del jugador.";
L["Format"] = "Formato";
L["Frame"] = "Cuadro";
L["Frequent Updates"] = "Actualizaciones Frecuentes";
L['Friendlies: Show Buffs'] = "Amistosos: Mostrar Beneficios";
L['Friendlies: Show Debuffs'] = "Amistosos: Mostrar Perjuicios";
L['Friendly Aura Type'] = "Tipo de Aura Amistosa";
L["Ghost"] = "Fantasma";
L["Good"] = "Bueno";
L["Group By"] = "Agrupar Por";
L["Group Point"] = "Punto de Agrupación";
L['Growth Direction'] = "Dirección de Crecimiento";
L["Heal Prediction"] = "Predicción de Sanación";
L["Health Backdrop"] = "Fondo de Salud";
L["Health By Value"] = "Salud por Valor";
L["Health Text"] = "Texto de Salud";
L["Health"] = "Salud";
L["Height"] = "Altura";
L['Hold shift + right click to blacklist this aura.'] = "Mantener shift + botón drecho para añadir este aura a la lista negra.";
L["Horizontal"] = "Horizontal"; --Also used in bags module
L["How far away the portrait is from the camera."] = "Como de lejos está el retrato de la cámara.";
L["Icon"] = "Icono";
L['If not set to 0 then override the size of the aura icon to this.'] = "Si no está a 0 entonces sobrescrive el tamaño del icono del aura con este.";
L['If set then if the aura is found on the blacklist filter it will not display. Note: You can change between only doing this on friendly or enemy units.'] = "Si se establece entonces si el aura se encuentra en la lista negra este no aparecerá. Nota: Puedes cambiar entre hacerlo solo en unidades amistosas o enemigas.";
L['If set then if the aura is found on the whitelist filter it will display. Note: You can change between only doing this on friendly or enemy units.'] = "Si se establece entonces si el aura se encuentra en la lista blanca este aparecerá. Nota: Puedes cambiar entre hacerlo solo en unidades amistosas o enemigas.";
L['If set, only auras belonging to yourself in addition to any aura that passes the set filter may be shown. Note: You can change between only doing this on friendly or enemy units.'] = "Si se establece, solo se mostrarán las auras pertenecientes a ti mismo además de cualquier aura que pase el filtro. Nota: Puedes cambiar entre hacerlo solo en unidades amistosas o enemigas.";
L["Interrupt Color"] = "Color de Interrupción";
L["Latency"] = "Latencia";
L["Low Mana Threshold"] = "Umbral de Bajo Maná";
L["Main statusbar texture."] = "Textura de la barra de estado principal.";
L["Match Frame Width"] = "Coincidir con la Anchura del Cuadro";
L["Max Columns"] = "Columnas Máximas";
L["MT, MA First"] = "TP, AP Primero";
L["Name"] = "Nombre"; --Also used in Buffs and Debuffs
L["Neutral"] = "Neutral";
L['No Consolidated'] = "No Consolidados"
L['No Duration'] = "No Duración";
L["None"] = "Ninguno"; --Also used in chat
L["Not valid spell id"] = "No es un id de hechizo valido";
L["Num Rows"] = "Numero de Filas";
L["Offline"] = "Fuera de Línea";
L["Offset of the powerbar to the healthbar, set to 0 to disable."] = "Desplazamiento de la barra de poder sobre la barra de salud, 0 para desactivar.";
L["Offset"] = "Desplazamiento";
L["OOR Alpha"] = true;
L["Orientation"] = "Orientación";
L["Overlay the healthbar"] = "Recubrir la barra de salud";
L["Overlay"] = "Recubrir";
L["Override any custom visibility setting in certain situations, EX: Only show groups 1 and 2 inside a 10 man instance."] = "Sobreescribir cualquier opción de visibilidad en ciertas situaciones, Ej: Solo mostrar grupos 1 y 2 dentro de una mazmorra de banda de 10 personas.";
L["Party Frames"] = "Cuadro de Grupo";
L["Party Pets"] = "Mascotas de Grupo";
L["Party Targets"] = "Objetivos del Grupo";
L["Per Row"] = "Por Fila";
L["Percent"] = "Porcentaje";
L["Personal Auras"] = "Auras Personales";
L["Pet Frame"] = "Cuadro de Mascota";
L["PetTarget Frame"] = "Cuadro de Objetivo de Mascota";
L["Player Frame"] = "Cuadro de Jugador";
L["Portrait"] = "Retrato";
L["Position"] = "Posición";
L["Power text will be hidden on NPC targets, in addition the name text will be repositioned to the power texts anchor point."] = "Texto de poder estará oculto en los objetivos PNJ, además el texto del nombre será posicionado al punto de anchaje del texto de poder.";
L["Power"] = "Poder";
L["Powers"] = "Poderes";
L["Priority"] = "Prioridad";
L['PVP Trinket'] = "Abalorio JcJ";
L['Raid-10 Frames'] = "Cuadro de Banda-10";
L['Raid-25 Frames'] = "Cuadro de Banda-25";
L['Raid-40 Frames'] = "Cuadro de Banda-40";
L["RaidDebuff Indicator"] = "Indicador de Perjuicios de Banda";
L["Rapidly update the health, uses more memory and cpu. Only recommended for healing."] = "Actualizar la salud rapidamente, consume más memoria y cpu. Recomendado solo para sanadores.";
L["Reactions"] = "Reacciones";
L["Remaining"] = "Restante";
L["Remove a spell from the filter."] = "Elimina un hechizo del filtro.";
L["Remove Spell"] = "Eliminar Hechizo";
L["Remove SpellID"] = "Eliminar ID de Hechizo";
L["Rest Icon"] = "Icono de Restauración";
L["Restore Defaults"] = "Restaurar por Defecto"; --Also used in Media and ActionBars sections
L['RL / ML Icons'] = "Iconos LB / MD";
L["Role Icon"] = "Icono de Rol";
L["Select a filter to use."] = "Selecciona un filtro a usar."; --Also used in NamePlates
L["Select a unit to copy settings from."] = "Seleciona una unidad desde la que copiar configuración.";
L["Select Filter"] = "Seleccionar Filtro";
L["Select Spell"] = "Seleccionar Hechizo";
L["Set the filter type, blacklisted filters hide any aura on the like and show all else, whitelisted filters show any aura on the filter and hide all else."] = "Establece el tipo de filtro, los filtros de lista negra ocultan cualquier aura en la lista y muestra el resto, la  lista blanca muestra los auras de la lista y oculta el resto.";
L["Set the font size for unitframes."] = "Establece el tamaño de la fuente para los cuadros de unidad.";
L["Set the order that the group will sort."] = "Establece el orden en que el grupo será ordenado.";
L["Set the priority order of the spell, please note that prioritys are only used for the raid debuff module, not the standard buff/debuff module. If you want to disable set to zero."] = "Establece el orden de prioridad del hechizo, ten en cuenta que la prioridad solo se usa para el modulo de perjuicios de banda, no para el modulo estandar de beneficios/perjuicios. 0 para desactivar.";
L['Set the type of auras to show when a unit is a foe.'] = "Establece el tipo de auras a mostrar cuando la unidad es enemigo";
L['Set the type of auras to show when a unit is friendly.'] = "Establece el tipo de auras a mostrar cuando la unidad es amistoso";
L["Show a incomming heal prediction bar on the unitframe. Also display a slightly different colored bar for incoming overheals."] = "Muestra una barra de predicción de sanación en el cuadro de unidad. Tambien muestra una barra ligeramente coloreada para sobresanaciones recividas.";
L['Show Auras'] = "Mostrar Auras";
L["Show Missing"] = "Mostrar Faltantes";
L["Show Party"] = "Mostrar Grupo";
L["Show Raid"] = "Mostrar Banda";
L["Show Solo"] = "Mostrar Solo";
L["Size of the indicator icon."] = "Tamaño del icono indicador.";
L['Size Override'] = "Sobreescribir Tamaño";
L["Size"] = "Tamaño";
L['Smart Auras'] = "Auras Pequeñas";
L["Smart Raid Filter"] = "Filtro de Banda Pequeño";
L["Smooth Bars"] = "Barras Suavizadas";
L["Spaced"] = "Espaciadas";
L["Spark"] = "Desatar";
L['Spec Icon'] = "Icono de Especialidad";
L["Spell not found in list."] = "Hechizo no encontrado en la lista.";
L["StatusBar Texture"] = "Textura de la Barra de Estado";
L["Tank Frames"] = "Cuadro de Tanques";
L['Tank Target'] = "Objetivo del Tanque";
L["Tapped"] = "Gopear";
L["Target Frame"] = "Cuadro de Objetivo";
L["TargetTarget Frame"] = "Cuadro de Objetivo de Objetivo";
L["Text Format"] = "Formato de Texto";
L["Text Toggle On NPC"] = "Alternar Texto en PNJ";
L["The alpha to set units that are out of range to."] = "Establece la transparencia para las unidades fuera de rango.";
L["The amount of space (in pixels) between the columns."] = "La cantidad de espacio (en píxeles) entre las columnas.";
L["The anchor point for each new column. A value of LEFT will cause the columns to grow to the right."] = "El punto de anclaje para cada nueva columna. Un valor de IZQUIERDA podria causar que crezca hacia la derecha.";
L["The following macro must be true in order for the group to be shown, in addition to any filter that may already be set."] = "La siguiente macro debe ser cierta en orden para que el grupo se muestre, ademas de cualquier filtro que ya exista.";
L["The font that the unitframes will use."] = "La fuente que el cuadro de unidad usa.";
L["The maximum number of columns that the header will create."] = "El número máximo de oclumnas que la cabecera creará.";
L["The maximum number of units that will be displayed in a single column."] = "El número máximo de unidades que serán mostradas en una sola columna.";
L['The name you have selected is already in use by another element.'] = "El nombre que has seleccionado ya está en uso por otro elemento.";
L['The object you want to attach to.'] = "El objeto que quieres adjuntar a.";
L['The point of which you are anchoring to in relation to the attach to object.'] = "El punto en que estás anclando en relacción al enlace con el objeto.";
L['The spell "%s" has been added to the Blacklist unitframe aura filter.'] = "El hechizo '%s' ha sido añadido a la Lista Negra del filtro de auras del cuadro de unidad.";
L["Ticks"] = "Ticks";
L["Toggles health text display"] = "Muestra/Oculta el texto de salud";
L["UnitFrames"] = "Cuadro de Unidad";
L["Units Per Column"] = "Unidades por Columna";
L["Up"] = "Arriba";
L['Use Blacklist'] = "Usar Lista Negra";
L["Use Filter"] = "Usar Filtro";
L["Use the custom health backdrop color instead of a multiple of the main health color."] = "Usar el color de fondo personalizado para la salud en vez de un multiplo del color principal.";
L['Use Whitelist'] = "Usar Lista Blanca";
L["Value must be a number"] = "El valor debe ser un número";
L["Vertical"] = "Vertical"; --Also used in bags section
L["Visibility"] = "Visibilidad";
L["What each frame should attach itself to, example setting it to TOP every unit will attach its top to the last point bottom."] = "Que cada cuadro debería adjuntarse consigo mismo, por ejemplo estableciento ARRIBA cada unidad se adjuntara a su último punto de abajo.";
L["What point to anchor to the frame you set to attach to."] = true;
L["What to attach the buff anchor frame to."] = true;
L["What to attach the debuff anchor frame to."] = true;
L['When set the Buffs and Debuffs will toggle being displayed depending on if the unit is friendly or an enemy. This will not effect the aurabars module.'] = "Cuando estableces los Beneficios y Perjuicios estos se mostraran dependiendo de si la unidad es amiga o enemiga. Esto no tiene efecto sobre las barras de auras.";
L["When true, the group header is shown when the player is in a party."] = "Cuando está activo, la cabecera del grupo se muestra cuando el jugador está en un grupo.";
L["When true, the group header is shown when the player is in a raid."] = "Cuando está activo, la cabecera del grupo se muestra cuando el jugador está en una banda.";
L["When true, the header includes the player when not in a raid."] = "Cuando está activo, la cabecera incluye al jugador cuando no está en una banda.";
L["When true, the header is shown when the player is not in any group."] = "Cuando está activo, la cabecera se muestra cuando no está en ningun grupo.";
L["When you mana falls below this point, text will flash on the player frame."] = "Cuando tu maná cae por debajo de este punto, el texto parpadeará en el cuadro del jugador.";
L["Whitelist"] = "Lista Blanca";
L["Width"] = "Anchura"; --Also used in NamePlates module
L["xOffset"] = "DesplazamientoX";
L["yOffset"] = "DesplazamientoY";
L["You can't remove a pre-existing filter."] = "No puedes eliminar un filtro pre-existente.";
L["You cannot copy settings from the same unit."] = "No puedes copiar la configuración desde la misma unidad";
L["You cannot set the Group Point and Column Point so they are opposite of each other."] = "No puedes establecer el punto de grupo y el punto de columna ya que uno es totalmente opuesto al otro";
L["You may not remove a spell from a default filter that is not customly added. Setting spell to false instead."] = "No puedes eliminar un hechizo de un filtro por defecto que no ha sido personalizado. Establece el hechizo a falso.";
L['TEXT_FORMAT_DESC'] = [=[Proporciona una cadena para cambiar el formato de texto.


Ejemplos:
[namecolor][name] [difficultycolor][smartlevel] [shortclassification]
[healthcolor][health:current-max]
[powercolor][power:current]


Formatos de Salud / Poder:
'current' - cantidad actual
'percent' - cantidad porcentual




'current-max' - cantidad actual seguido de cantidad máxima, solo se mostrará la máxima si la actual es igual a la máxima
'current-percent' - cantidad actual seguido de porcentaje
'current-max-percent' - cantidad actual, cantidad máxima y porcentaje, solo se mostrará la máxima si la actual es igual a la máxima
'deficit' - muestra el varol de deficit, no muestra nada si no hay deficit





Formatos de Nombre:
'name-short' - Nombre restringido a 10 caracteres
'name-medium' - Nombre restringido a 15 caracteres
'name-long' - Nombre restringido a 20 caracteres


Para desactivarlo dejar el campo en blanco, si necesitas más información visita http://www.tukui.org]=];